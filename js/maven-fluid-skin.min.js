 ...
 */
(function(f){var d={};f.event.special.imageready={setup:function(a,b,c){d=a||d},add:function(c){var b=f(this),a;if(this.nodeType===1&&this.tagName.toLowerCase()==="img"&&this.src!==""){if(d.forceLoad){a=b.attr("src");b.attr("src","");e(this,c.handler);b.attr("src",a)}else{if(this.complete||this.readyState===4){c.handler.apply(this,arguments)}else{e(this,c.handler)}}}},teardown:function(a){f(this).unbind(".imageready")}};function e(c,a){var b=f(c);b.bind("load.imageready",function(){a.apply(c,arguments);b.unbind("load.imageready")})}}(jQuery));new function(b){b.fn.placeholder=function(t){t=t||{};var l=t.dataKey||"placeholderValue";var p=t.attr||"placeholder";var n=t.className||"placeholder";var a=t.values||[];var s=t.blockSubmit||false;var q=t.blankSubmit||false;var o=t.onSubmit||false;var m=t.value||"";var r=t.cursor_position||0;return this.filter(":input").each(function(c){b.data(this,l,a[c]||b(this).attr(p))}).each(function(){if(b.trim(b(this).val())===""){b(this).addClass(n).val(b.data(this,l))}}).focus(function(){if(b.trim(b(this).val())===b.data(this,l)){b(this).removeClass(n).val(m)}if(b.fn.setCursorPosition){b(this).setCursorPosition(r)}}).blur(function(){if(b.trim(b(this).val())===m){b(this).addClass(n).val(b.data(this,l))}}).each(function(d,c){if(s){new function(e){b(e.form).submit(function(){return b.trim(b(e).val())!=b.data(e,l)})}(c)}else{if(q){new function(e){b(e.form).submit(function(){if(b.trim(b(e).val())==b.data(e,l)){b(e).removeClass(n).val("")}return true})}(c)}else{if(o){new function(e){b(e.form).submit(o)}(c)}}}})}}(jQuery);jQuery(document).ready(function(c){function d(a){c("form.custom input:"+a).each(function(){var b=c(this).hide(),f=b.next("span.custom."+a);if(f.length===0){f=c('<span class="custom '+a+'"></span>').insertAfter(b)}f.toggleClass("checked",b.is(":checked"))})}d("checkbox");d("radio");c("form.custom select").each(function(){var h=c(this),a=h.next("div.custom.dropdown"),j=h.find("option"),i=0,b;if(a.length===0){a=c('<div class="custom dropdown"><a href="#" class="selector"></a><ul></ul></div>"');j.each(function(){b=c("<li>"+c(this).html()+"</li>");a.find("ul").append(b)});a.prepend('<a href="#" class="current">'+j.first().html()+"</a>");h.after(a);h.hide()}j.each(function(e){if(this.selected){a.find("li").eq(e).addClass("selected");a.find(".current").html(c(this).html())}});a.find("li").each(function(){a.addClass("open");if(c(this).outerWidth()>i){i=c(this).outerWidth()}a.removeClass("open")});a.css("width",i+18+"px");a.find("ul").css("width",i+16+"px")})});(function(d){function e(c){var a=c.prev(),b=a[0];b.checked=((b.checked)?false:true);c.toggleClass("checked");a.trigger("change")}function f(c){var a=c.prev(),b=a[0];d('input:radio[name="'+a.attr("name")+'"]').each(function(){d(this).next().removeClass("checked")});b.checked=((b.checked)?false:true);c.toggleClass("checked");a.trigger("change")}d(document).on("click","form.custom span.custom.checkbox",function(a){a.preventDefault();a.stopPropagation();e(d(this))});d(document).on("click","form.custom span.custom.radio",function(a){a.preventDefault();a.stopPropagation();f(d(this))});d(document).on("click","form.custom label",function(c){var h=d("#"+d(this).attr("for")),a,b;if(h.length!==0){if(h.attr("type")==="checkbox"){c.preventDefault();a=d(this).find("span.custom.checkbox");e(a)}else{if(h.attr("type")==="radio"){c.preventDefault();b=d(this).find("span.custom.radio");f(b)}}}});d(document).on("click","form.custom div.custom.dropdown a.current, form.custom div.custom.dropdown a.selector",function(c){var a=d(this),b=a.closest("div.custom.dropdown");c.preventDefault();b.toggleClass("open");if(b.hasClass("open")){d(document).bind("click.customdropdown",function(h){b.removeClass("open");d(document).unbind(".customdropdown")})}else{d(document).unbind(".customdropdown")}});d(document).on("click","form.custom div.custom.dropdown li",function(b){var a=d(this),i=a.closest("div.custom.dropdown"),c=i.prev(),j=0;b.preventDefault();b.stopPropagation();a.closest("ul").find("li").removeClass("selected");a.addClass("selected");i.removeClass("open").find("a.current").html(a.html());a.closest("ul").find("li").each(function(g){if(a[0]==this){j=g}});c[0].selectedIndex=j;c.trigger("change")})})(jQuery);
/*! http://mths.be/placeholder v1.8.5 by @mathias */
(function(d,e,b){var c="placeholder" in e.createElement("input"),f="placeholder" in e.createElement("textarea");if(c&&f){b.fn.placeholder=function(){return this};b.fn.placeholder.input=b.fn.placeholder.textarea=true}else{b.fn.placeholder=function(){return this.filter((c?"textarea":":input")+"[placeholder]").bind("focus.placeholder",g).bind("blur.placeholder",a).trigger("blur.placeholder").end()};b.fn.placeholder.input=c;b.fn.placeholder.textarea=f;b(function(){b("form").bind("submit.placeholder",function(){var h=b(".placeholder",this).each(g);setTimeout(function(){h.each(a)},10)})});b(d).bind("unload.placeholder",function(){b(".placeholder").val("")})}function p(h){var j={},i=/^jQuery\d+$/;b.each(h.attributes,function(l,k){if(k.specified&&!i.test(k.name)){j[k.name]=k.value}});return j}function g(){var h=b(this);if(h.val()===h.attr("placeholder")&&h.hasClass("placeholder")){if(h.data("placeholder-password")){h.hide().next().show().focus().attr("id",h.removeAttr("id").data("placeholder-id"))}else{h.val("").removeClass("placeholder")}}}function a(){var i,h=b(this),j=h,l=this.id;if(h.val()===""){if(h.is(":password")){if(!h.data("placeholder-textinput")){try{i=h.clone().attr({type:"text"})}catch(k){i=b("<input>").attr(b.extend(p(this),{type:"text"}))}i.removeAttr("name").data("placeholder-password",true).data("placeholder-id",l).bind("focus.placeholder",g);h.data("placeholder-textinput",i).data("placeholder-id",l).before(i)}h=h.removeAttr("id").hide().prev().attr("id",l).show()}h.addClass("placeholder").val(h.attr("placeholder"))}else{h.removeClass("placeholder")}}}(this,document,jQuery));$(document).ready(function(){function a(e){var d=e.closest("dl").find("a.active"),c=e.attr("href")+"Tab";d.removeClass("active");e.addClass("active");$(c).closest(".tabs-content").children("li").hide();$(c).show()}$("dl.tabs").each(function(){var c=$(this).children("dd").children("a");c.click(function(d){a($(this))})});if(window.location.hash){a($('a[href="'+window.location.hash+'"]'))}$(".alert-box").delegate("a.close","click",function(c){c.preventDefault();$(this).closest(".alert-box").fadeOut(function(d){$(this).remove()})});$("input, textarea").placeholder();var b=null;$(".nav-bar li a, .nav-bar li a:after").each(function(){$(this).data("clicks",0)});$(".nav-bar li a, .nav-bar li a:after").live("click",function(d){d.preventDefault();if(b!==$(this).index()||b===null){$(this).data("clicks",0);b=$(this).index()}$(this).data("clicks",($(this).data("clicks")+1));var c=$(this).siblings(".flyout");if(!c.is(":visible")&&$(this).parent(".has-flyout").length>1){$(".nav-bar li .flyout").hide();c.show()}else{if(($(this).data("clicks")>1)||($(this).parent(".has-flyout").length<1)){window.location=$(this).attr("href")}}});$(".nav-bar").live("click",function(c){c.stopPropagation();if($(c.target).parents().is(".flyout")||$(c.target).is(".flyout")){c.preventDefault()}})});